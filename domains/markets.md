...
**Domain owner:** Market Brain agent | **Last updated:** 2026-07-11
...
- The admin publish flow's `source_check` gate fails on any `trends.google.com` Resolution Source URL — confirmed by testing both the full query-string comparison URL and a bare `trends.google.com/trends/explore?geo=XX` URL, both rejected. Comic Con's non-Trends source URL passed cleanly, isolating the cause to the Trends domain itself (likely Google blocking server-side/bot fetches, not a URL-formatting issue). Any Attention Battle market resolving on Google Trends will hit this until it's fixed or worked around — don't burn time rewriting the query string, save as Draft and flag it (2026-07-11)
- Admin form date/time segment entry has a reproducible off-by-one focus bug: after typing a segment, focus sometimes advances two segments instead of one (e.g. typing the hour lands in the minute field, leaving hour unset), rather than the previously-documented corruption-only failure mode. Always verify each segment individually via zoom immediately after typing before moving on — don't assume tab-order advances correctly (2026-07-11)
